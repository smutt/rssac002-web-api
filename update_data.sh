...
/usr/local/bin/wget --no-host-directories --max-redirect=0 --retry-on-host-error --recursive --https-only --no-clobber --no-parent https://root-servers.org/archives/ 2>&1 |egrep -v '^$'|grep -v "already there"
